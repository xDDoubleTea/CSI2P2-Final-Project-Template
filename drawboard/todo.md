...
- [x] **Garbage System**
  - [x] Implement `GarbageQueue` to store incoming attacks.
  - [x] Implement `addGarbageLine(int count)` in `Board.cpp`.
  - [x] Render garbage lines (Grey color, one hole per line).
  - [x] Implement garbage cancellation (clearing lines reduces incoming garbage).
  - [x] Implement "Cheese" mechanics (garbage lines rising from bottom).
- [ ] **Enemy Interaction**
...
- [x] **APP (Attack Per Piece)**
  - [x] Add `APP` field to `Stat.h`.
  - [x] Calculate `APP = Total Attack / Pieces Placed`.
  - [x] Display APP in `Stat::draw`.
